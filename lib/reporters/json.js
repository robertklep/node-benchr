module.exports = runner => {
  // Handle output options.
  let prettyPrint = runner.options.prettyPrint;
  let progress    = runner.options.progress;
  let verbose     = runner.options.verbose;

  // Handle runner events.
  let currentFile, currentSuite, stats = [];
  runner.on('run.start', file => {
    progress && process.stderr.write('R');
  }).on('file.start', file => {
    progress && process.stderr.write('F');
    currentFile = {
      name   : file,
      suites : [],
    };
    stats.push(currentFile);
  }).on('file.complete', file => {
    progress && process.stderr.write('f');
  }).on('suite.start', suite => {
    progress && process.stderr.write('S');
    currentSuite = {
      name       : suite.name,
      benchmarks : [],
    };
    currentFile.suites.push(currentSuite);
  }).on('suite.complete', suite => {
    progress && process.stderr.write('s');

    // Handle fastest/successful benchmarks.
    let fastest          = suite.filter('fastest');
    let successful       = suite.filter('successful');
    currentSuite.fastest = (successful.length > 1 && fastest.length > 0 && fastest.length !== successful.length) ?
                            suite.filter('fastest').map('name')[0] :
                            null;

    // Handle benchmark statistics.
    currentSuite.benchmarks = suite.map(bench => {
      if (verbose) return bench;
      delete bench.stats.sample;
      return {
        name    : bench.name,
        hz      : bench.hz,
        aborted : bench.aborted,
        stats   : bench.stats,
      };
    });
  }).on('benchmark.start', () => {
    progress && process.stderr.write('B');
  }).on('benchmark.complete', () => {
    progress && process.stderr.write('b');
  }).on('run.complete', () => {
    progress && process.stderr.write('r\n');
    if (prettyPrint) {
      console.log(JSON.stringify(stats, null, 2));
    } else {
      console.log('%j', stats);
    }
  })
};
