...
Runner.prototype.addSuite = function(name, fn) {
  ...
  suite.on('start', function() {
    ...
    this.forEach(function(bench) {
      ...
      table.cell('name', chalk[ bench.aborted ? 'red' : 'green' ](bench.name + ': '));
      if (bench.aborted) {
        table.cell('hz', chalk.red('ABORTED'));
      } else {
        table.cell('hz', chalk.green(bench.hz.toFixed(2).toString().replace(/\B(?=(\d{3})+\b)/g, ',')), function(str, width) {
          return Table.padLeft(str, width);
        });
        table.cell('label',  chalk.green('ops/sec'));
        table.cell('rme',    '±' + bench.stats.rme.toFixed(2) + '%');
        table.cell('runs',   '(' + bench.stats.sample.length + ' runs sampled)');
      }
      table.newRow();
    });
    console.log('\n\n' + table.print());
